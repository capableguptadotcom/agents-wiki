...
const assemblyLayers = [
  {
    ...
    standards: ["W3C Trace Context for correlation", "host app identity", "domain UI contracts"],
    cases: ["Slack agents", "Atlassian Rovo", "GitHub coding agent"],
  ...
  },
  {
    ...
    standards: ["OIDC claims", "SMART launch context", "FHIR Encounter when healthcare", "domain source references"],
    cases: ["Copilot Studio channels", "Agentforce data grounding", "healthcare command centers"],
  ...
  },
  {
    ...
    standards: ["OAuth 2.0", "OIDC", "SMART scopes", "MCP authorization", "OWASP excessive agency"],
    cases: ["Microsoft Copilot Studio governance", "ServiceNow workflow permissions"],
  ...
  },
  {
    ...
    standards: ["ReAct trajectories", "Toolformer-style tool traces", "OpenAI Agents SDK", "Vercel AI SDK", "LangGraph"],
    cases: ["Vercel AI SDK agents", "OpenAI Agents SDK", "LangGraph deep agents"],
  ...
  },
  {
    ...
    standards: ["MCP latest specification", "OpenAPI", "JSON Schema", "FHIR resources", "CloudEvents"],
    cases: ["Agentforce actions", "ServiceNow AI Agent Studio", "Cloudflare Agents tools"],
  ...
  },
  {
    ...
    standards: ["NIST AI RMF oversight", "Vercel human-in-the-loop", "LangGraph interrupts", "Temporal workflow messages"],
    cases: ["Vercel HITL examples", "enterprise workflow approval inboxes"],
  ...
  },
  {
    ...
    standards: ["Temporal workflows", "Inngest functions", "Cloudflare Workflows", "CloudEvents"],
    cases: ["ServiceNow workflows", "Cloudflare Agents with Workflows"],
  ...
  },
  {
    ...
    standards: ["FHIR", "SMART App Launch", "calendar APIs", "CRM APIs", "git and CI APIs"],
    cases: ["healthcare command centers", "Agentforce CRM grounding", "GitHub PR checks"],
  ...
  },
  {
    ...
    standards: ["Reflexion", "Generative Agents", "Voyager", "OWASP memory poisoning risks", "NIST GenAI Profile"],
    cases: ["LangGraph memory", "Cloudflare Agents state", "enterprise custom instructions"],
  ...
  },
  {
    ...
    standards: ["OpenTelemetry GenAI semantic conventions", "W3C Trace Context", "audit-control requirements"],
    cases: ["Sentry Seer-style investigation", "GitHub coding-agent logs", "ServiceNow control tower patterns"],
  ...
  },
  {
    ...
    standards: ["AgentBench", "WebArena", "GAIA", "NIST AI RMF", "ISO/IEC 42001"],
    cases: ["GitHub coding-agent review loops", "ServiceNow AI Control Tower positioning"],
    deepDive: "eval-release-harness.html"
  }
];

const assemblyScenarios = {
  bed: {
    label: "Healthcare bed flow",
...
const deepDives = [
  ["End-to-end assembly", "Use the Architecture Assembly lab to trace one real request across surface, context, authority, agent loop, capability gateway, approval, workflow, source truth, memory, observability, and release."],
...
let currentAssemblyScenario = "bed";
let currentAssemblyLayer = "surface";
...
function assemblyRecordPreview(scenarioKey, layerKey) {
  const scenario = assemblyScenarios[scenarioKey] || assemblyScenarios.bed;
  const layer = assemblyLayers.find((item) => item.key === layerKey) || assemblyLayers[0];
  const detail = scenario.layers[layer.key];
  return {
    assembly_record_id: `assembly_${scenarioKey}_${layer.key}`,
    scenario: scenario.label,
    ...
    layer: layer.label,
    owner: layer.owner,
    product_record: detail.record,
    upstream_dependency: detail.upstream,
    downstream_consumer: detail.downstream,
    invariant: detail.invariant,
    standards_and_theory: layer.standards,
    industry_patterns: layer.cases,
    failure_to_prevent: detail.failure,
    release_test: detail.test,
    implementation_next_step: detail.implementation,
    companion_deep_dive: layer.deepDive
  };
}

function renderAssemblyWalkthrough(scenarioKey = currentAssemblyScenario, layerKey = currentAssemblyLayer) {
  currentAssemblyScenario = scenarioKey;
  currentAssemblyLayer = layerKey;
  const scenario = assemblyScenarios[scenarioKey] || assemblyScenarios.bed;
  const layer = assemblyLayers.find((item) => item.key === layerKey) || assemblyLayers[0];
  const detail = scenario.layers[layer.key];

  document.getElementById("assembly-scenarios").innerHTML = Object.entries(assemblyScenarios).map(([key, item]) => `
    <button class="assembly-scenario-button ${key === scenarioKey ? "active" : ""}" data-assembly-scenario="${key}">
      <strong>${item.label}</strong>
      <span>${item.domain}</span>
    </button>
  `).join("");

  document.getElementById("assembly-layers").innerHTML = assemblyLayers.map((item, index) => {
    const layerDetail = scenario.layers[item.key];
    return `
      <button class="assembly-layer-button ${item.key === layer.key ? "active" : ""}" data-assembly-layer="${item.key}">
        <span>${index + 1}</span>
        <strong>${item.label}</strong>
        <small>${layerDetail.record}</small>
      </button>
    `;
  }).join("");

  document.getElementById("assembly-detail").innerHTML = `
    <div class="assembly-kicker">
      <span>${scenario.label}</span>
      <span>${layer.owner}</span>
      <span>${detail.record}</span>
    </div>
    <h3>${layer.label}</h3>
    <p class="assembly-request"><strong>User intent:</strong> ${scenario.intent}</p>
    <p><strong>Layer theory:</strong> ${layer.theory}</p>
    <div class="assembly-detail-grid">
      ...
      <div class="assembly-card">
        <strong>Upstream dependency</strong>
        <p>${detail.upstream}</p>
      </div>
      <div class="assembly-card">
        <strong>Downstream handoff</strong>
        <p>${detail.downstream}</p>
      </div>
      <div class="assembly-card">
        <strong>Invariant</strong>
      ...
      <div class="assembly-card warning">
        <strong>Failure if missing</strong>
        <p>${detail.failure}</p>
      </div>
      <div class="assembly-card">
    ...
    <article class="assembly-test">
      <strong>Release test</strong>
      <span>${detail.test}</span>
    </article>
  `;

  document.getElementById("assembly-record").textContent = JSON.stringify(
    assemblyRecordPreview(scenarioKey, layer.key),
    null,
    2
  );

  document.getElementById("assembly-path").innerHTML = assemblyLayers.map((item, index) => {
    const layerDetail = scenario.layers[item.key];
    return `
      <button class="assembly-path-step ${item.key === layer.key ? "active" : ""}" data-assembly-layer="${item.key}">
        <strong>${index + 1}. ${item.label}</strong>
        <span>${layerDetail.record}</span>
      </button>
    `;
  }).join("");

  document.getElementById("assembly-anchors").innerHTML = `
    <div class="assembly-anchor-card">
      <strong>Scenario source truth</strong>
    ...
    <div class="assembly-anchor-card">
      <strong>Standards and theory</strong>
      <ul>${layer.standards.map((item) => `<li>${item}</li>`).join("")}</ul>
    </div>
    <div class="assembly-anchor-card">
      <strong>Industry patterns</strong>
      <ul>${layer.cases.map((item) => `<li>${item}</li>`).join("")}</ul>
    </div>
    <div class="assembly-anchor-card">
      <strong>Deep dive</strong>
      <p><a href="${layer.deepDive}">${layer.deepDive}</a></p>
    </div>
  `;
}
...
document.addEventListener("click", (event) => {
  ...
  const assemblyScenario = event.target.closest(".assembly-scenario-button");
  if (assemblyScenario) renderAssemblyWalkthrough(assemblyScenario.dataset.assemblyScenario, currentAssemblyLayer);

  const assemblyLayer = event.target.closest(".assembly-layer-button, .assembly-path-step");
  if (assemblyLayer && assemblyLayer.dataset.assemblyLayer) {
    renderAssemblyWalkthrough(currentAssemblyScenario, assemblyLayer.dataset.assemblyLayer);
  }
