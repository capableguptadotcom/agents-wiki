...
This page defines the canonical visual model for the wiki. These diagrams are not decoration. Each diagram must unlock an engineering decision.
...
Every side effect must be reconstructable from `AgentRun`, `ToolCall`, `PolicyDecision`, `Approval`, `WorkflowEvent`, `SourceResponse`, `VerificationResult`, `TimelineEvent`, and `AuditEvent`.
...
No UI may show `completed` from model text. Completion requires source confirmation or an explicit human reconciliation record.
...
The sequence must pass with a fake bed hold before any broader agent platform work starts.
...
The model is not an enforcement boundary. Enforcement lives in context binding, capability grants, policy, approval, workflow, source adapters, audit, and release gates.
...
Every write-side effect needs `run_id`, `tool_call_id`, `policy_decision_id`, `approval_id` when applicable, `workflow_id`, `source_response_id`, `audit_id`, and `trace_id`.
...
Approval authorizes only the canonical payload hash that was shown to the approver. Any modification re-enters policy.
